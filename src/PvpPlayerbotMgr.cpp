...
public:
    PvpPlayerbotLoginQueryHolder(PvpPlayerbotHolder* pvpPlayerbotHolder, uint32 masterAccount, uint32 accountId, ObjectGuid guid)
...
void PvpPlayerbotHolder::HandleBotPackets(WorldSession* session)
{
    WorldPacket* packet;
...
void PvpPlayerbotHolder::LogoutAllBots()
{
    /*
    while (true)
    {
...
void PvpPlayerbotMgr::CancelLogout()
{
...
void PvpPlayerbotHolder::LogoutPlayerBot(ObjectGuid guid)
{
...
void PvpPlayerbotHolder::DisablePlayerBot(ObjectGuid guid)
{
    if (Player* bot = GetPlayerBot(guid))
    {
        PlayerbotAI* botAI = GET_PVPPLAYERBOT_AI(bot);
...
std::string const PvpPlayerbotHolder::ProcessBotCommand(std::string const cmd, ObjectGuid guid, ObjectGuid masterguid,
...
bool PvpPlayerbotMgr::HandlePvpPlayerbotMgrCommand(ChatHandler* handler, char const* args)
{
    ...
    std::vector<std::string> messages = mgr->HandlePvpPlayerbotCommand(args, player);
    if (messages.empty())
        return true;
...
std::vector<std::string> PvpPlayerbotHolder::HandlePvpPlayerbotCommand(char const* args, Player* master)
{
...
uint32 PvpPlayerbotHolder::GetAccountId(std::string const name) { return AccountMgr::GetId(name); }

uint32 PvpPlayerbotHolder::GetAccountId(ObjectGuid guid)
{
    if (!guid.IsPlayer())
        return 0;
...
std::string const PvpPlayerbotHolder::ListBots(Player* master)
{
...
std::string const PvpPlayerbotHolder::LookupBots(Player* master)
{
...
PvpPlayerbotMgr::PvpPlayerbotMgr(Player* const master) : PvpPlayerbotHolder(), master(master), lastErrorTell(0) {}
...
void PvpPlayerbotMgr::HandleCommand(uint32 type, std::string const text)
{
    Player* master = GetMaster();
    if (!master)
        return;
...
void PvpPlayerbotMgr::TellError(std::string const botName, std::string const text)
{
...
void PvpPlayerbotMgr::CheckTellErrors(uint32 elapsed)
{
...
PlayerbotAI* PvpPlayerbotsMgr::GetPvpPlayerbotAI(Player* player)
{
